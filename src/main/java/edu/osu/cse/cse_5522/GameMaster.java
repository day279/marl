...
public class GameMaster {
	...
	private List<List<Score>> window(List<Score> scores) {
		int numberOfGames = scores.size();
		int numberOfWindows = 10;
		int windowSize = (numberOfGames < numberOfWindows ? numberOfGames : numberOfGames/numberOfWindows);
		
		List<List<Score>> windowedScores = new ArrayList<List<Score>>();
		List<Score> currentWindow = null;

		for (int i = 0; i < numberOfGames; i++) {
			if (i % windowSize == 0) {
				currentWindow = new ArrayList<Score>();
				windowedScores.add(currentWindow);
			}
			currentWindow.add(scores.get(i));
		}
